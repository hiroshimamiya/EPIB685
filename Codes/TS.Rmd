...
$$ logE(Y_t) = log(\lambda_t) =  \beta_0 + \beta_{ozone} X_t + log(population)$$     
$\beta_0$ is intercept term       
$\beta_{ozone}$ is regression coefficient representing the association between ozone and mortality count. **It is critical to understand the interpretation of this variable when exponentiated**     
...
```{r}
ci.lin(model1,subset="ozone",Exp=T) %>%  round(digit = 3)
```
### ACF 
...
#### If you happen to approximate the outcome with a normal distribution (what condition are required for this?), we will also check the starndard assumptions linear regression, which is completely violated
```{r}
...
$$ logE(Y_t) = log(\lambda_t) =  \beta_0 + \beta_{ozone} X_t + S_t + \delta t, $$ 
...
ci.lin(model2,subset="ozone",Exp=T) %>%  round(digit = 3)
```
...
Lets check RR 
```{r}
ci.lin(model3,subset="ozone",Exp=T) %>%  round(digit = 3)
```
...
```{r}
acf(residuals(model3, type = "response"), main = "ACF of residual, after controlling for temperature")
```
...
Additionally, we will add lag to temperature too for a better ajustment.    
...
$$ log(\lambda_t) = \beta_0 + \beta_{ozone1} X_1 + \beta_{ozone2} X_2 + \beta_{ozone3} X_3 + ... + \beta_{ozone7} X_7, $$ 
and instead prefer to use a constrained non-linear function below    
$$ log(\lambda_t) = \beta_0 + f(x_{t};\beta), $$ 
...
```
### Settings for the temperature lag effect
```{r}
####################################
...
### Lets plot the lagged impact of ozone on mortality counts over days, in Relative Risk (RR). 
The plot shows the RR of Ozone on day $t$ on the same day ($t$), next day ($t+1$), day after ($t+3 ... t+7$)...so that we will allow ozone to influence mortality as a carryover effect over days, even after ozone level goes down.      
```{r}
#############
# FIGURE 4C
#############

plot(pred4_Ozone,var=10,type="p",ci="bars",col=1,pch=19,ylim=c(0.99,1.03),
...
plot(pred4_Temp,var=10,type="p",ci="bars",col=1,pch=19,ylim=c(0.5,1.5),
     main="All lag terms modelled together (with costraints)",xlab="Lag (days)",
     ylab="RR and 95%CI per 10ug/m3 ozone increase")
