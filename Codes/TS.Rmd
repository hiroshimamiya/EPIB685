...
# Make the outcome log count (so that we constrain to the positive value in interpretation of regression coefficients)
...
See changes when you add/remove the cyclic terms (Fourier and date) and trend from the previous model 
```{r}
...
plot(pred4_Ozone,var=10,type="p",ci="bars",col=1,pch=19,
     ...
     ylab="Change in log(y) and its 95%CI per 10ug/m3 ozone increase")
...
plot(pred4_Temp,var=10,type="p",ci="bars",col=1,pch=19,
     main="All lag terms modelled together (with costraints)",xlab="Lag (days)",
     ylab="Change in log(y) and its 95%CI per 1C temperature increase")
