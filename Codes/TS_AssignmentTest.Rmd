...
load("C:/Users/hiroshimamiya/OneDrive/Teaching/EPIB685/EPIB685_W2025/Assignments/Assignment3/tsdatasets_compilation/tsdatasets_location.RData")
bal <- dlist$balt.us8700 # Baltimore data 
val <- dlist$vale.sp0107
chi <- dlist$chic.us8700
...
plot(bal$date,bal$all,main="Daily deaths over time",
...
#################
summary(bal$pm10)
bal$pm10 <- bal$pm10/10

summary(bal$pm10) # double check scaled or not , new variable ozone10 
```
...
```{r}
model1 <- glm(all ~ pm10, bal,family=quasipoisson)  
ci.lin(model1,subset="pm10",Exp=T) %>%  round(digit = 3)
```
### ACF 
...
model2 <- glm(all ~ pm10 + fourier + time,bal,family=quasipoisson)
...
ci.lin(model2,subset="pm10",Exp=T) %>%  round(digit = 3)
```
...
model3 <- glm(all ~ pm10 + fourier + time + tempdecile, bal,family=quasipoisson)

```
Lets check RR 
```{r}
ci.lin(model3,subset="pm10",Exp=T) %>%  round(digit = 3)
```
...
```{r}
acf(residuals(model3, type = "response"), main = "ACF of residual, after controlling for temperature", na.action = na.pass)
```
...
model4 <- glm(all ~ cbpmconstr + cbtempunc + fourier + date , bal,family=quasipoisson)
...
pred4_temp <- crosspred(cbtempunc,model4,at=5, cumul = TRUE, bylag = 0.2)
...
plot(pred4_temp,var=5, type="p",ci="bars",col=1,pch=19,ylim=c(0.5,1.5), 
     main="All lag terms modelled together (with costraints)",xlab="Lag (days)",
...
model5 <- glm(all ~ cbpmconstr + cbtempunc + fourier + date + y_lag ,bal,family=quasipoisson)
...
cbtempunc <- crossbasis(bal$temperature,lag=c(0,7),
                        argvar=list(fun="strata",
                                    breaks=cutoffs[2:10]),
                        arglag=list(fun="integer"))

cbo3constr <- crossbasis(bal$ozone,lag=c(0,7),
                         argvar=list(fun="lin"),
...
model6<- lm(diff_y ~ cbo3constr + cbtempunc, bal)
