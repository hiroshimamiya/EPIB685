...
## **Question 1 -1**: Compare the two maps and describe any differences, and explan the source of differences.   
...
## **Question 1 -2** Compare the two maps below, state which map is more appropraite and why
```{r}
...
## **Question 1 -3**: Align the hospital (point data) and FSA  
...
## **Question 2-1** State which of the two map is appropriate to present the spatial distribution of income, and explain why.        
...
## **Question 2-2** See the red shaded map  below showing the rate of gi count per area in 2006, and explain what condition is needed to call these numbers annual "rate"? 
...
## **Question 2-3**  Based on the plot of SMR and its Variance, population size, and the crude disease counts below, answer qustions a-f below.      
  ...
  g) Base on your explanation in f), do you think that the SMR below is useful to describe the spatial occurrence of GI? 
...
## **Question 3-1.** Among the three types of spatial anlaysis, which one is closest to our current task and why?    

## **Question 3-2.** Create a variable in `fsaMtl` that stores the model-smoothed SMR.     
Hint: fitted count values from model are available as `fittedModel$fitted.values`. 

## **Question 3-3.** Map the smoothed SMR. Add a proper legend.        

## **Question 3-4.** Discribe the spatial distribution of model-smoothed SMR, in contrast to the un-smoothed original SMR. Which SMR surface is likely to be correct and why?     

## **Question 3-5.** How many areas have statistically significantly higher relative risk than the region-level risk (RR=1)?   
