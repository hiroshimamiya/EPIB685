...
# Time
### COdes borrowed from Dr. Antonio Gasparrini
## https://github.com/gasparrini/2013_bhaskaran_IJE_codedata/blob/master/londondataset2002_2006.dta
## some extra lines were  inserted to make the codes easier to follow 
...
# First install the packge - !! Once installed, remove the installation commande below or comment out by #, as there is no need to install the same lib again and again, all it takes is to use library() command to load installed library.  
...
# GENERATE spoecific varibles from MONTH AND YEAR 
...
plot(data$date,data$numdeaths,ylim=c(100,300),cex=1,col=grey(0.6),
...
# CONTROLLING FOR SEASONALITY (WITH SPLINE AS IN MODEL 3), plus ozone 
